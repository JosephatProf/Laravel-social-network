...
<div class="row">
	<div class="col-lg-6">
		...
		<form class="form-vertical" role="form" method="post" action="{{ route('profile.edit')}}" enctype="multipart/form-data">
	        ...
	        <input class="form-control" type="file" name="mypic">
	        </div>
			<div class="well">
		        @if(Storage::disk('local')->exists(Auth::user()->username.'-profile'.Auth::user()->id.'.jpg'))
		            <div >
		                <img class="img-thumbnail" src="{{ route('profile.edit.image',['filename'=>Auth::user()->username.'-profile'.Auth::user()->id.'.jpg'])}}" height="150px" width="150px">
		            </div>
		        @endif
		    </div>
			<div class="form-group">
